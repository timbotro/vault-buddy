...
import {ausd, ausdIbtcDexshare, dot, ibtc, intr, lcdot, ldot} from '../static/tokens'
...
export const setupAcala = async () => {
  ...
  const getDotIntrPrice = async (fp: boolean = false) => {
    ...
    const respDot = (await api.query.dex.liquidityPool([ausd, dot])).toJSON()!
    const intrPrice = respIntr[0] / respIntr[1]
    const dotPrice = respDot[0] / respDot[1]
  ...
  const printStats = async (intrHarvest, dotHarvest) => {
    ...
    console.log(
      `🧮 Acala KINT Price: ${intrInDot} DOT / $${intrInUsd.toFixed(2)}`
    )
    printPercentages(intrPrice, intrInUsd)
    console.log(
      `🌾 Harvestable Amount: ${intrHarvest} KINT / ${dotHarvest.toFixed(
        2
