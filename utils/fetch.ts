...
const acaUri = 'https://api.polkawallet.io/price-server/'
...
export const getAcaStatsPrice = async (asset: string) => {
  try {
    const resp = (await fetch(acaUri + `/?token=${asset}&from=market`, {
      method: 'GET',
    })) as any
    const json = await resp.json()
